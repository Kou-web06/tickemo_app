...
register(components_group, app_sources_phase, %w[
  ShareTicketShape.swift
  ShareTicketCardView.swift
  ShareCDCardView.swift
  ShareReceiptCardView.swift
  ShareBarcodeView.swift
])

screens_group = find_or_create_group(tickemo_group, "Screens", "Tickemo/Screens")
register(screens_group, app_sources_phase, %w[ShareSheetView.swift])

support_group = find_or_create_group(tickemo_group, "Support", "Tickemo/Support")
register(support_group, app_sources_phase, %w[ShareCardData.swift ShareCapture.swift ShareActions.swift])
...
register(tests_group, test_sources_phase, %w[ShareCardDataTests.swift])
